...
| Platform | Cache Type | TTL Support | Implementation |
|----------|-----------|-------------|----------------|
| Cloudflare Workers | Edge KV Store | 7 days | `caches.default` API |
| Vercel Edge | Vercel CDN | 7 days | `s-maxage`, `stale-while-revalidate` |
| Netlify Edge | Netlify CDN | 7 days | `Netlify-CDN-Cache-Control` |
| Nginx | Proxy Cache | 7 days | `proxy_cache_path` |
| Apache | Disk Cache | 7 days | `mod_cache_disk` |
| AWS CloudFront | Edge Locations | 7 days | Cache Policy |

### Cache Flow

```
Bot Request → Edge Cache Check
                    │
           ┌───────┴───────┐
           │               │
       HIT (95%)      MISS (5%)
           │               │
      Return HTML    Fetch from HypoText
      (10-50ms)            │
                     Store in Cache
                           │
                     Return HTML
...
| Metric | Without Cache | With Edge Cache | Improvement |
|--------|--------------|-----------------|-------------|
| Response Time | 200-500ms | 10-50ms | 90% faster |
| Origin Requests | 100% | 5-10% | 90% reduction |
| Bandwidth Cost | Full | 40-60% saved | 50% savings |
| API Server Load | 100% | 5-10% | 90% reduction |

### Cost Savings with Edge Caching

#### 100 Sites (100M visits/month)

| Component | Without Cache | With Cache | Savings |
|-----------|--------------|------------|---------|
| App Runner | $570 | $114 | $456 (80%) |
| Data Transfer | $450 | $45 | $405 (90%) |
| S3 Requests | $4 | $0.40 | $3.60 (90%) |
| **Total** | **$1,455** | **$590** | **$865 (59%)** |

#### 1,000 Sites (1B visits/month)

| Component | Without Cache | With Cache | Savings |
|-----------|--------------|------------|---------|
| App Runner | $4,555 | $911 | $3,644 (80%) |
| RDS | $2,189 | $438 | $1,751 (80%) |
| Data Transfer | Included | Included | - |
| **Total** | **$11,360** | **$6,030** | **$5,330 (47%)** |

### Recommended TTL by Content Type

| Content Type | TTL | Reason |
|--------------|-----|--------|
| Static pages | 7 days | Matches weekly render schedule |
| Blog posts | 7 days | Rarely updated |
| Product pages | 1-3 days | Price/stock changes |
| Dynamic content | 1 day | Frequent updates |

### Cache Invalidation

```javascript
// Invalidate on page re-render (webhook from HypoText)
// Cloudflare
...
*Document generated for HypoText infrastructure planning*
*All prices based on AWS us-east-1 region as of February 2026*
